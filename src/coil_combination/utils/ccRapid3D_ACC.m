function [cc_img3D, coilmaps3D] = ccRapid3D_ACC(input_img ,bg_mask, sig_patch, fov, nloc3D, sl_batch, accel, use_nroi)

[Nx, Ny, Nsl, Ncon, Nch] = size(input_img);


% Use a defined ROI to calculate the signal interference matrix
...
% fprintf('Coil combination: now processing %02d%%', 0);
...
for sl = 1:Nsl
    ...
    % fprintf('\b\b\b%02d%%',floor( ((sl-1)*Nx_new+i)/(Nsl*Nx_new)*100) );
