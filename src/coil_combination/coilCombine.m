function [ccImg, varargout] = coilCombine(img, dimStr, combineMethod, varargin)
...
%     'noisePatchLoc': The interference patches required for 'BF' and 
%                      'MVDR'. The dimension for 'noisePatchLoc' should be
%                      (Nroi, x&y coordinates, Nsl). 
...
%  >> [ccImg] = coilCombineRapid3D(img, 'xycnz', 'SOS')
% 
% If you want to do adaptive coil combine and has data dimension 
% (Nx,Ny,Nsl,Nch) = (256,256,10,15).
%  >> [ccImg, coilmaps] = coilCombineRapid3D(img, 'xyzc', 'ACC', 'patchSize', [13,13], 'fov', [128,128])
%
% If you want to do beamforming on data dimension (Nx,Ny,Nch,Nsl,Ncon) 
% = (256,256,15,10,6), and this is a 2D multislice data. You will need to
% use 'a' for the slice dimension so that each slice will be calculated
% separately.
%  >> % need to have nLoc with dimension (2,4,10)
%  >> [ccImg, coilmaps] = coilCombineRapid3D(img, 'xycan', 'ACC', 'noisePatchLoc', nLoc, 'fov', [128,128])
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
elseif strcmp(combineMethod, 'ACC')
    ...
    use_nroi = 0; % Do not use interference patches
    if Nrpt == 1
        [ccImg, coilmaps3D] = ccRapid3D_ACC(img, bgMask, patchSize, fov, noisePatchLoc, slBatch, accel, use_nroi);
        ...
        for r = 1:Nrpt
            imgTemp = img(:,:,:,:,:,r);
            [ccImg(:,:,:,:,:,r), coilmaps3D(:,:,:,:,:,r)] = ccRapid3D_ACC(imgTemp, bgMask, patchSize, fov, noisePatchLoc, slBatch, accel, use_nroi);
        end
    end
end
