% =========================================================================
% Script: Demo_pig_experiment.m
% Author: Qing Dai
% Date: 2025-12-16
%
% Description:
%   This script demonstrates Active EMI Suppression (AES) on in vivo pig
%   data acquired during microwave ablation on a 3T scanner.
%
%
...
tiledlayout(1,3);
nexttile; 
imshow3(abs(kdata_rx_decor(:,2:2:end,:)), kdataDisplayRange, shape);
title('Baseline');
nexttile; 
imshow3(abs(kdata_rx_decor_aes(:,2:2:end,:)), kdataDisplayRange, shape);
title('Proposed AES');
nexttile; 
imshow3(abs(kdata_diff(:,2:2:end,:)), kdataDisplayRange, shape);
title('Complex Difference');


% Plot 2: Single Coil Images
imgDisplayRange = [0 5e-7];
